...
/**
 * REST API handler to cancel an active Claude session by provider or conduit session id.
 * POST /api/claude/session/:sessionId/cancel
 */
export async function cancelSession(c: Context) {
	try {
		...
		const result = cancelHeadlessCliSession({ cli: 'claude', sessionId });
