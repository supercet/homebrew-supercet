...
function executeClaudeCodeSession(options: {
  ...
  errorPrefix: string;
  streamCallback?: (data: { type: 'stdout' | 'stderr' | 'sessionId' | 'complete' | 'error'; content: string }) => void;
}): Promise<ClaudeCodeSession> {
  const { prompt, workingDir, spawnArgs, initialSessionId, extractSessionId: shouldExtractSessionId, errorPrefix, streamCallback } = options;
